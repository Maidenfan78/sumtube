...
def transcript_from_youtube(url: str) -> Optional[str]:
    ...
    try:
        txt = YouTubeTranscriptApi.get_transcript(
            vid, languages=["en", "en-US", "auto"]
        )
...
async def summarise(text: str, model="gpt-4o") -> str:
    ...
    seg_summaries = [
        r.choices[0].message.content.strip() for r in await asyncio.gather(*tasks)
    ]
    combo = "\n".join(seg_summaries)
    final = await client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Condense to a single paragraph (≈120 words).",
            },
